...
import { askQuestion, deleteDocument, listDocuments, uploadDocuments } from "./api/ragApi";
...
import { getAnswerScopeLabel, mapApiDocument } from "./utils/documents";

const emptyWorkspaceForm = { name: "", description: "", category: "General", tags: "" };
const defaultWorkspace = {
  id: "default",
  name: "Knowledge Base",
  description: "Uploaded documents indexed in Supabase pgvector for grounded AI answers.",
  category: "General",
  status: "Active",
  documentCount: 0,
  lastActivity: "--",
...
function App() {
  ...
  const [documentsByWorkspace, setDocumentsByWorkspace] = useState({ [defaultWorkspace.id]: [] });
  ...
  const refreshDocuments = async () => {
    try {
      const documents = (await listDocuments()).map(mapApiDocument);
      setDocumentsByWorkspace({ [defaultWorkspace.id]: documents });
        ...
        current.map((workspace) =>
          workspace.id === defaultWorkspace.id
            ? {
                ...
                lastActivity: documents[0]?.uploaded || "--",
              }
            : workspace,
        ),
      );
  ...
  const uploadPendingFiles = async () => {
    ...
    try {
      ...
      showSnackbar("Document uploaded and indexed.");
    } catch (error) {
